...
# Palette colori e configurazioni universali
...
# --- FUNZIONE API (CON BLOCCO EXCEPTION PER DEBUG) ---
def get_gmaps_info(origin, destination):
    try:
        ...
        # Visualizza l'errore esatto a schermo per il programmatore
...
def run_dispatch(df_c, df_f):
    ...
    for _, riga in df_c.iterrows():
        ...
        for f_idx, aut in df_f.iterrows():
            ...
            # Logica Efficienza: accoppia passeggeri con stessa rotta
            ...
            if is_pooling:
                best_aut_idx = f_idx
                match_info = {'pronto': riga['DT_Richiesta'], 'vuoto': 0, 'da': "Pooling con gruppo"}
                break
...
if 'risultati' not in st.session_state:
    ...
    if f_c and f_f:
        if st.button("ELABORA CRONOPROGRAMMA", type="primary", use_container_width=True):
    ...
    color_map = {d: COLORS[i % len(COLORS)] for i, d in enumerate(unique_drivers)}
    ...
    with c_aut:
        ...
        for _, r in df[df['Autista'] == sel_aut].iterrows():
            with st.expander(f"Corsa {r['ID']} - Partenza {r['Partenza']}", expanded=True):
