...
st.markdown("""
    ...
    .main-title { text-align: center; color: #1E1E1E; margin-bottom: 30px; }
    </style>
...
if 'res_df' not in st.session_state:
    ...
    st.markdown(f'<h2 class="main-title">📊 Dashboard Operativa - {datetime.now().strftime("%d/%m/%Y")}</h2>', unsafe_allow_html=True)
    
    # BOX AUTISTI
    cols = st.columns(len(flotta))
    for i, (_, row) in enumerate(flotta.iterrows()):
        color = df[df['Autista'] == row[0]]['Color'].iloc[0] if row[0] in df['Autista'].values else "gray"
        cols[i].markdown(f'<div class="driver-box" style="background-color:{color};"><b>{row[0]}</b><br><span class="stat-val">{row["Servizi"]}</span><br>Servizi</div>', unsafe_allow_html=True)

    # TABELLE RENDICONTO
    t1, t2 = st.tabs(["📋 Rendiconto Cliente", "🚘 Diario Autisti"])
    with t1:
        st.dataframe(df[['ID', 'Da', 'A', 'Inizio', 'Fine', 'Status', 'Note']].style.apply(lambda x: [f"background-color: {x.Color}; color: white" for _ in x], axis=1), use_container_width=True)
    with t2:
        for aut in flotta.iloc[:, 0]:
            with st.expander(f"Programma per {aut}"):
                st.table(df[df['Autista'] == aut][['ID', 'Da', 'A', 'Inizio', 'Fine', 'Status']])

    if st.button("🔄 CARICA NUOVI FILE"):
        del st.session_state['res_df']; st.rerun()

    # Punto 1: Esportazione Excel
    output = io.BytesIO()
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        df.to_excel(writer, index=False, sheet_name='Piano_Viaggi')
    st.download_button("📥 Scarica Rendiconto Completo (Excel)", output.getvalue(), "EmiTrek_Piano.xlsx")