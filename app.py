...
# --- SISTEMA DI ACCESSO (FIX LOGIN ISTANTANEO) ---
def check_password():
    if "password_correct" not in st.session_state:
        ...
        with col_btn_login:
            # Rimosso st.form per evitare il bug del doppio click
            if st.button("✨ ENTRA NEL SISTEMA"):
                if u in st.secrets["users"] and p == st.secrets["users"][u]["password"]:
                    st.session_state["password_correct"] = True
                    st.rerun() # Forza il caricamento immediato al primo click
                else:
...
def get_gmaps_info(origin, destination):
    ...
    try:
        if "MAPS_API_KEY" not in st.secrets: return 30, "Chiave assente"
    ...
    except Exception: return 30, "Stima"
    return 30, "Stima"

def run_dispatch(df_c, df_f):
    """Assegnazione intelligente con saturazione flotta"""
    ...
    df_f['Posizione_Attuale'] = "BASE"; df_f['Servizi_Count'] = 0; df_f['Last_Trip_Time'] = pd.NaT; df_f['Passeggeri_Caricati'] = 0
    risultati = []
    ...
    for _, corsa in df_c.iterrows():
        ...
        cap_max = CAPACITA.get(tipo_v, 3); best_aut_idx = None; min_punteggio = float('inf'); match_info = {}
        autisti_idonei = df_f[df_f['Tipo Veicolo'].str.capitalize() == tipo_v]

        for idx, autista in autisti_idonei.iterrows():
            # 1. Car Pooling
            is_pooling = (autista['Posizione_Attuale'] == corsa['Destinazione Finale'] and not pd.isna(autista['Last_Trip_Time']) and
                          abs((autista['Last_Trip_Time'] - corsa['DT_Richiesta']).total_seconds()) <= 300 and autista['Passeggeri_Caricati'] < cap_max)
            if is_pooling:
                best_aut_idx = idx; match_info = {'pronto': corsa['DT_Richiesta'], 'da': "Car Pooling", 'vuoto': 0, 'ritardo': 0}; break

            # 2. Tempi (Spostamento + 15m accoglienza)
            if autista['Servizi_Count'] == 0: minuti_vuoto = 0; ora_pronto = corsa['DT_Richiesta']
            else:
            ...
            bonus_saturazione = 5000 if autista['Servizi_Count'] > 0 else 0 # Priorità a chi già lavora
            punteggio = (ritardo * 5000) + minuti_vuoto - bonus_saturazione
            if punteggio < min_punteggio:
                min_punteggio = punteggio; best_aut_idx = idx
        ...
        if best_aut_idx is not None:
            ...
            risultati.append({
                'Autista': df_f.at[best_aut_idx, 'Autista'], 'ID': corsa['ID Prenotazione'], 'Mezzo': df_f.at[best_aut_idx, 'ID Veicolo'], 'Tipo': tipo_v,
                'Prelievo': corsa['Indirizzo Prelievo'], 'Partenza': partenza_effettiva, 'Destinazione': corsa['Destinazione Finale'], 'Arrivo': arrivo_effettivo,
                'Status': "🟢 OK" if match_info['ritardo'] <= 2 else f"🔴 RITARDO {int(match_info['ritardo'])} min",
                'M_Vuoto': match_info['vuoto'], 'M_Pieno': minuti_pieno, 'Provenienza': match_info['da'], 'Ritardo_Val': int(match_info['ritardo'])
            })
            df_f.at[best_aut_idx, 'DT_Disponibile'] = arrivo_effettivo; df_f.at[best_aut_idx, 'Posizione_Attuale'] = corsa['Destinazione Finale']; df_f.at[best_aut_idx, 'Last_Trip_Time'] = corsa['DT_Richiesta']; df_f.at[best_aut_idx, 'Servizi_Count'] += 1; df_f.at[best_aut_idx, 'Passeggeri_Caricati'] += 1
...
if check_password():
    ...
    st.markdown('<p class="sub-title">Pianificazione ottimizzata flotta NCC</p>', unsafe_allow_html=True)

    if 'risultati' not in st.session_state:
        st.write("### 📂 Carica i file della giornata")
        col_f1, col_f2 = st.columns(2)
        with col_f1: file_prenotazioni = st.file_uploader("📋 Lista Prenotazioni (.xlsx)", type=['xlsx'])
        with col_f2: file_flotta = st.file_uploader("🚘 Flotta Disponibile (.xlsx)", type=['xlsx'])
        if file_prenotazioni and file_flotta:
            ...
            with col_btn_go:
                if st.button("🚀 ORGANIZZA I VIAGGI DI OGGI"):
                    ...
                    st.session_state['risultati'], st.session_state['flotta_finale'] = df_res, df_flotta_agg; st.rerun()
    else:
        df_res, df_flotta = st.session_state['risultati'], st.session_state['flotta_finale']
        ...
        for i, (_, autista_row) in enumerate(df_flotta.iterrows()):
            ...
            with box_cols[i]:
                st.markdown(f'<div style="background-color:{driver_colors[nome]}; padding:20px; border-radius:15px; text-align:center; color:white;"><small>{nome}</small><br><b style="font-size:22px;">{autista_row["Tipo Veicolo"]}</b><br><hr style="margin:10px 0; border:0; border-top:1px solid rgba(255,255,255,0.3);"><span style="font-size:16px;">Servizi: {autista_row["Servizi_Count"]}</span></div>', unsafe_allow_html=True)

        st.divider()
        st.subheader("🗓️ Tabella di Marcia")
        df_display = df_res.copy(); df_display['Ora Inizio'] = df_display['Partenza'].dt.strftime('%H:%M'); df_display['Ora Fine'] = df_display['Arrivo'].dt.strftime('%H:%M')
        st.dataframe(df_display[['Autista', 'ID', 'Mezzo', 'Prelievo', 'Ora Inizio', 'Destinazione', 'Ora Fine', 'Status']].style.apply(lambda x: [f"background-color: {driver_colors.get(x.Autista)}; color: white; font-weight: bold" for _ in x], axis=1), use_container_width=True)
        ...
        with col_d1:
            ...
            for _, r in df_res[df_res['Autista'] == sel_autista].iterrows():
                with st.expander(f"Corsa {r['ID']} - Ore {r['Partenza'].strftime('%H:%M')}", expanded=False):
                    if r['Ritardo_Val'] > 0: st.error(f"⚠️ RITARDO RILEVATO: {r['Ritardo_Val']} minuti!")
                    st.write(f"📍 Proviene da: **{r['Provenienza']}**")
                    st.write(f"⏱️ Tempo prelievo: **{r['M_Vuoto']} min** + 15m accoglienza")
                    st.write(f"⏱️ Tempo viaggio: **{r['M_Pieno']} min** + 15m scarico")
                    st.write(f"✅ Libero alle: **{r['Arrivo'].strftime('%H:%M')}**")
        with col_d2:
