...
# --- 1. CONFIGURAZIONE ESTETICA E BOX COLORATE ---
...
def ai_validate_time(origin, dest, g_min):
    """L'AI non improvvisa: analizza il contesto reale per validare Google Maps"""
    try:
        ...
        prompt = (f"Sei un esperto logistico NCC a Roma. Da {origin} a {dest}, Google stima {g_min} min. "
                  f"Considerando il traffico reale di Roma e i tempi di carico, fornisci una stima precisa. "
                  f"Rispondi SOLO con il numero intero dei minuti, senza testo aggiuntivo.")
        
        response = model.generate_content(prompt)
        res_text = ''.join(filter(str.isdigit, response.text))
        return int(res_text) if res_text else g_min, True
    except:
        return g_min, False

def get_real_metrics(origin, destination):
    """Recupera i dati reali dalle API di Google Maps"""
    try:
        ...
        if res:
            ...
            return final_t, dist_text, ai_active
    except:
        return 0, "Errore API", False
...
def run_dispatch(df_c, df_f):
    ...
    for i, (_, r) in enumerate(df_c.iterrows()):
        status.text(f"📡 Richiesta dati reali per Corsa {r['ID']}...")
        ...
        # 1. SMART POOLING (ACCORPAMENTO PER REQUISITO)
        ...
        for idx_p, aut_p in potenziali.iterrows():
            ...
            if abs((last['Partenza'] - r['DT_R']).total_seconds()) <= 600:
                res_list.append({
                    'Autista': aut_p['Autista'], 'ID': r['ID'], 'Mezzo': aut_p['ID Veicolo'], 'Veicolo': req,
        ...
        if not assigned:
            ...
            for idx, aut in idonei.iterrows():
                dv, _, _ = get_real_metrics(aut['Pos'], r['Indirizzo Prelievo'])
            ...
            if best_m:
                ...
                dp, dist, ai_p = get_real_metrics(r['Indirizzo Prelievo'], r['Destinazione Finale'])
                partenza = max(r['DT_R'], op)
                arrivo = partenza + timedelta(minutes=dp + 10) # Scarico 10m
                res_list.append({
                    'Autista': df_f.at[idx, 'Autista'], 'ID': r['ID'], 'Mezzo': df_f.at[idx, 'ID Veicolo'], 'Veicolo': req,
...
if check_password():
    ...
    if 'risultati' not in st.session_state:
        ...
        st.divider()
        st.subheader("🗓️ Tabella di Marcia (Dati Reali)")
        ...
        with ca:
            ...
            for _, r in df[df['Autista'] == sel].iterrows():
                with st.expander(f"Corsa {r['ID']} - Ore {r['Inizio']}"):
                    ...
                    st.write(f"🛣️ **Distanza:** {r.get('Dist', 'N/D')}")
                    if r['AI']: st.info("🤖 Tempo validato da Vertex AI per precisione traffico.")
        with cc:
