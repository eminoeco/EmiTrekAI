...
if not st.session_state['processed_data']:
    ...
    # --- SEQUENZA OPERATIVA UNIFICATA – VERSIONE FINALE 100% FUNZIONANTE ---
    ...
    if assigned_df.empty:
        ...
        # DataFrame finale – sicuro anche se mancano colonne
        display_df = pd.DataFrame({
            'Autista'       : assigned_df['Autista Assegnato'].fillna('—'),
            'Cliente'Cliente'       : assigned_df.get('ID Prenotazione', pd.Series('—', index=assigned_df.index)),
            'Partenza'      : assigned_df.get('Indirizzo Prelievo', pd.Series('—', index=assigned_df.index)).fillna('—'),
