...
# --- CONFIGURAZIONE ---
st.set_page_config(layout="wide", page_title="EmiTrekAI | SaaS Universal Dispatcher", page_icon="🚐")
pd.options.mode.chained_assignment = None

# Colori generici per gli autisti (assegnati dinamicamente)
...
def get_gmaps_info(origin, destination):
    try:
        # Usa il nome salvato nei tuoi Secrets
        ...
        if res:
            ...
            strade = [s for s in steps if any(k in s for k in ["Via", "Viale", "Raccordo", "Autostrada"])]
            return durata, f"{' ➡️ '.join(list(dict.fromkeys(strade))[:3])} ({distanza})"
    except Exception:
        return 20, "Calcolo GPS Standard"
    return 20, "Percorso non trovato"
...
def run_dispatch(df_c, df_f):
    ...
    for idx, riga in df_c.iterrows():
        ...
        # LOGICA DI ACCORPAMENTO UNIVERSALE (Grouping)
        # Funziona con qualsiasi indirizzo caricato nell'Excel
        ...
        for f_idx, aut in df_f.iterrows():
            ...
            # Calcolo basato sulla posizione attuale dinamica del mezzo
        ...
        if best_aut_idx is not None:
            ...
            for g_idx in gruppo.index:
                res_list.append({
                    ...
                    'Note': f"Gruppo di {len(gruppo)} persone" if len(gruppo) > 1 else "Singolo"
                })
                assegnati.add(g_idx)
...
# Gestione sessione per la sparizione dei file
if 'res_c' not in st.session_state:
    st.subheader("📂 Caricamento Dati Dinamico")
    c1, c2 = st.columns(2)
    with c1: f_c = st.file_uploader("Prenotazioni Clienti", type=['xlsx'])
    with c2: f_f = st.file_uploader("Disponibilità Flotta", type=['xlsx'])
    
    if f_c and f_f:
        if st.button("ELABORA PIANO OPERATIVO", type="primary", use_container_width=True):
    ...
    # Colori dinamici basati sugli autisti presenti nell'Excel
    ...
    with col_aut:
        ...
        for _, job in rc[rc['Autista'] == sel_d].iterrows():
            with st.expander(f"🕒 Servizio {job['ID']} - Ore {job['Partenza']}", expanded=True):
                st.write(f"🔄 Proviene da: **{job['Provenienza']}** | ⏱️ Vuoto: {job['Vuoto_Min']} min")

    with col_cli:
        st.header("📍 Dettaglio Cliente")
        sel_c = st.selectbox("Seleziona ID Prenotazione:", rc['ID'].unique())
        c_i = rc[rc['ID'] == sel_c].iloc[0]
        st.info(f"🛣️ **Percorso Reale Google:** {c_i['Itinerario']}")