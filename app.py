...
# --- FUNZIONE API (Parametro key= fondamentale) ---
def get_gmaps_info(origin, destination):
    try:
        api_key = st.secrets["GOOGLE_MAPS_API_KEY"]
        gmaps = googlemaps.Client(key=api_key) # Ecco il parametro key=
        ...
        return 20, f"Errore API: {str(e)[:30]}"
...
def run_dispatch(df_c, df_f):
    ...
    for idx, riga in df_c.iterrows():
        ...
        gruppo = df_c[(~df_c.index.isin(assegnati)) & (df_c['Indirizzo Prelievo'] == riga['Indirizzo Prelievo']) & (df_c['Destinazione Finale'] == riga['Destinazione Finale']) & (df_c['DT_Richiesta'] == riga['DT_Richiesta'])].head(cap_max)
    ...
    with col_aut:
        ...
        for _, job in rc[rc['Autista'] == sel_d].iterrows():
            with st.expander(f"🕒 Servizio {job['ID']} - Ore {job['Partenza']}", expanded=True):
                st.write(f"🔄 Da: {job['Provenienza']} | ⏱️ Vuoto: {job['Vuoto_Min']} min | 🏁 Arrivo: {job['Arrivo_Effettivo']}")
    with col_cli:
        st.header("📍 Analisi Clienti")
        sel_c = st.selectbox("Seleziona Cliente:", rc['ID'].unique())
        c_i = rc[rc['ID'] == sel_c].iloc[0]
        st.info(f"🛣️ **Itinerario:** {c_i['Itinerario']}\n\n✅ **Nota AI:** Anticipo di {c_i['Anticipo']} min.")