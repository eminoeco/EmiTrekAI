...
def get_gmaps_info(origin, destination):
    try:
        # Sincronizzato con il nome nei tuoi Secrets online
        if "Maps_API_KEY" not in st.secrets:
            return 30, "ERRORE: Nome chiave non trovato nei Secrets"
...
def run_dispatch(df_c, df_f):
    ...
    for _, riga in df_c.iterrows():
        ...
        for f_idx, aut in df_f.iterrows():
            ...
            # Logica Pooling: assegna lo stesso autista se orario e meta coincidono
...
if 'risultati' not in st.session_state:
    ...
    st.subheader("🗓️ Cronoprogramma Clienti (Ogni riga un cliente)")
    ...
    with c_cli:
        ...
        sel_id = st.selectbox("Seleziona ID Prenotazione:", df['ID'].unique())
