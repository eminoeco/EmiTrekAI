...
if "VERTEX_READY" not in st.session_state:
    try:
        ...
        st.error(f"Errore inizializzazione Vertex: {e}")
...
def get_metrics_real(origin, dest):
    try:
        ...
        # Validazione AI per evitare tempi "improvvisati"
...
def run_dispatch(df_c, df_f):
    ...
    # Rilevamento automatico delle colonne (FIX NAMEERROR)
    ...
    for i, (_, r) in enumerate(df_c.iterrows()):
        ...
        for idx, f in df_f.iterrows():
            ...
            df_f.at[idx, "DT_OBJ"] = arr
            df_f.at[idx, "Pos"] = r[col_c_dest]
            break
    progress_bar.empty()
    return pd.DataFrame(res)

if check_password():
    st.title("🚐 EmiTrekAI | Smart Dispatch Reale")
    f1 = st.file_uploader("Carica Prenotazioni (Excel)", type=['xlsx'])
    f2 = st.file_uploader("Carica Flotta (Excel)", type=['xlsx'])

    if f1 and f2:
        if st.button("🚀 AVVIA DISPATCH DINAMICO"):
            with st.spinner("Interrogazione API Google Maps + Vertex AI in corso..."):
                df_risultato = run_dispatch(pd.read_excel(f1), pd.read_excel(f2))
                st.success("Dispatch completato con successo!")
